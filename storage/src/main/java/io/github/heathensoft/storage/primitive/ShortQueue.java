...
import io.github.heathensoft.storage.primitive.iterators.ShortReader;
...
public class ShortQueue implements ReadableShort {
    
    private short[] q;
    private int f,r,p;
    
    public ShortQueue(int cap) {
     ...
     */
    public void enqueue(short[] i) {
        if (i != null) { makeRoom(i.length);
            for (short v : i) enqueue(v);
        }
    }
     ...
     */
    public void enqueue(Collection<Short> c) {
        if (c != null) { makeRoom(c.size());
            for (Short i : c) enqueue(i);
        }
    }
    ...
    public void dequeueAll(ShortReader itr) {
